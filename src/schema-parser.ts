...
type GetTypeFromSchema = (schema: JsonSchema, rootSchema: JsonSchema, registry?: SchemaRegistry) => string;
...
const getTypeFromSchema: GetTypeFromSchema = (schema, rootSchema, registry) => {
  ...
  if (schema.oneOf) {
    ...
    combinatorType = generateIntersectionType(schema.allOf, rootSchema, registry);
  }

  // 3. Check for Object Properties or explicit object type
  const hasObjectDefinition = schema.properties ||
...
type GenerateUnionType = (schemas: JsonSchema[], rootSchema: JsonSchema, registry?: SchemaRegistry) => string;

const generateUnionType: GenerateUnionType = (schemas, rootSchema, registry) => {
  const types = schemas.map(s => getTypeFromSchema(s, rootSchema, registry));
  return types.join(' | ');
}

type GenerateIntersectionType = (schemas: JsonSchema[], rootSchema: JsonSchema, registry?: SchemaRegistry) => string | null;

const generateIntersectionType: GenerateIntersectionType = (schemas, rootSchema, registry) => {
  const types = schemas.map(s => getTypeFromSchema(s, rootSchema, registry));
...
type ResolveLegacyRef = (ref: string, rootSchema: JsonSchema, registry?: SchemaRegistry) => string;

const resolveLegacyRef: ResolveLegacyRef = (ref, rootSchema, registry) => {
   // Legacy Logic
  const resolved = resolveRefObject(ref, rootSchema);
  if (resolved && Object.keys(resolved).length > 0) {
    if (resolved.type && !resolved.properties && !resolved.items && !resolved.oneOf) {
...
type GenerateObjectType = (schema: JsonSchema, rootSchema: JsonSchema, registry?: SchemaRegistry) => string;

const generateObjectType: GenerateObjectType = (schema, rootSchema, registry) => {
  if (!schema.properties && !schema.additionalProperties) {
  ...
  if (schema.additionalProperties) {
    if (typeof schema.additionalProperties === 'object') {
       const valueType = getTypeFromSchema(schema.additionalProperties, rootSchema, registry);
...
const toPascalCase: ToPascalCase = (str) => {
  return str.replace(/[^a-zA-Z0-9]/g, ' ').replace(/(?:^|\s)(\w)/g, (_, char) => char.toUpperCase()).replace(/\s/g, '');
}
...
type ShouldUseTypeAlias = (schema: JsonSchema, typeDefinition: string) => boolean;
...
const shouldUseTypeAlias: ShouldUseTypeAlias = (schema, typeDefinition) => {
  ...
  if (['string', 'number', 'boolean', 'array'].includes(schema.type as string)) return true;
